...
def map_to_party(row):
    ...
    # 1. Front National
    if any(x in liste for x in ["front national", "français d'abord", "france française", "liste entente populaire et nationale"]):
        return "Front National"
    if "front national" in text or " fn " in text:
        return "Front National"

    # 2. PCF
    if any(x in liste for x in ["communiste", "forces de gauche", "transformation à grande vitesse"]):
        return "PCF"
    if "parti communiste" in text or " pcf " in text:
        return "PCF"

    # 3. PS
    if any(x in liste for x in ["majorité présidentielle", "france unie", "mitterrand", "union de la gauche", "socialiste"]):
        return "PS"
    if "parti socialiste" in text or " ps " in text:
        return "PS"

    # 4. RPR / LR (Main Right)
    if any(x in liste for x in ["rassemblement pour la république", "union du rassemblement et du centre", "union pour la france", "majorité de la france", "union pour une nouvelle majorité"]):
        return "RPR / LR"
    if " rpr " in text or "rassemblement pour la république" in text:
        return "RPR / LR"

    # 5. UDF / Centre
    if any(x in liste for x in ["démocratie française", "udf", "union centriste", "rassemblement démocrate", "ouverture au centre"]):
        return "UDF / Centre"
    if " udf " in text or "union pour la démocratie française" in text:
        return "UDF / Centre"

    # 6. Ecologistes
    if any(x in liste for x in ["ecologie", "verts", "environnement", "génération ecologie"]):
        return "Ecologistes"
    if "écologiste" in text or " les verts " in text:
        return "Ecologistes"

    # 7. Extreme Gauche
    if any(x in liste for x in ["lutte ouvrière", "lcr", "parti des travailleurs", "autogestion", "révolutionnaire"]):
        return "Extreme Gauche"

    # 8. Extreme Droite / Nationalistes (Non-FN)
    if any(x in liste for x in ["nationaliste", "opposition nationale", "force nationale"]):
