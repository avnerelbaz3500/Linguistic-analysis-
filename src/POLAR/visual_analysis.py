...
def main():
    ...
    plt.title("Evolution of 'Langue de Bois' in French Political Speeches\n(Score toward +1 = Wooden Language | Score toward -1 = Direct Speech)", fontsize=16, pad=20)
    ...
    order = df.groupby('affiliate political party')['ldb_score'].median().sort_values(ascending=False).index
    ...
    plt.title("Repartition of 'Langue de Bois' by political party (1981-1993)\n(Score toward +1 = Wooden Language | Score toward -1 = Direct Speech)", fontsize=16)
    ...
    sns.barplot(data=df, x='ldb_score', y='titulaire-sexe', palette="viridis", hue="titulaire-sexe", errorbar=None)
    plt.title("'langue de bois' according to the gender \n(Highest scores closest to +1)", fontsize=16)
    plt.xlabel("POLAR score", fontsize=12)
    ...
    labels = ['18-29 ans', '30-39 ans', '40-49 ans', '50-59 ans', '60-69 ans', '70 ans et +']
    ...
    plt.title("'Langue de bois' according to age group\n(Highest scores closest to +1)", fontsize=16)
    plt.xlabel("POLAR score", fontsize=12)
    ...
    if 'titulaire-nom' in df.columns and 'titulaire-prenom' in df.columns: 
        ...
        plt.title("Top 10 most wooden language speeches\n(Highest scores closest to +1)", fontsize=16)
        plt.xlabel("POLAR score", fontsize=12)
