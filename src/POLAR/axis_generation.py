...
def validate_and_fix_pairs(pairs: list, expected_length: int) -> list:
    ...
    for item in pairs:
        ...
        if "direct" not in item or "langue_de_bois" not in item:
            raise ValueError(
                orange("Missing required keys 'direct' or 'langue_de_bois'.")
            )
        ...
        if re.search(r"[\u4e00-\u9fff\u3040-\u30ff\uac00-\ud7af]", text_content):
            raise ValueError(
                orange("Detected non-French (Asian) characters in the output.")
            )

        # Rebuild dict to strip any extra unwanted keys the LLM might have added
        valid_pairs.append(
            {"direct": item["direct"], "langue_de_bois": item["langue_de_bois"]}
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Iterative dataset generator for political rhetoric."
    )
    parser.add_argument(
        "-e",
        "--engine",
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Number of successful generation loops to complete.",
    )
    parser.add_argument(
        "-p",
        "--pairs_per_iter",
        type=int,
        default=3,
        help="Number of pairs requested per prompt.",
    )
    parser.add_argument(
        "-r",
        "--max_retries",
        type=int,
        default=3,
        help="Maximum attempts per iteration if JSON formatting fails.",
    )
    parser.add_argument(
        "-o",
        "--output",
    ...
    parser.add_argument(
        "--temp", type=float, default=0.8, help="Generation temperature."
    )
    parser.add_argument(
        "--verbose", "-v", action="store_true", help="display more information"
    )
        ...
        for attempt in range(args.max_retries):
            ...
            if clean_json_str:
                try:
                    ...
                    print(
                        f"Success: {len(new_pairs)} valid pairs added (Total: {len(dataset)})."
                    )
