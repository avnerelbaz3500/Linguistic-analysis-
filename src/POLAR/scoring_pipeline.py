...
def build_polar_axis(pairs_path: Path, tokenizer, model, device) -> torch.Tensor:
    ...
    embed_direct = embed_sentences(direct_texts, tokenizer, model, device) # (num_sentences, hidden_size)
    embed_ldb = embed_sentences(ldb_texts, tokenizer, model, device) # (num_sentences, hidden_size)
...
def compute_polar_scores_bulk(sentences: list, tokenizer, model, device, polar_vector, batch_size=32) -> np.ndarray:
    ...
    for i in tqdm(range(0, len(sentences), batch_size), desc="Computing sentence scores"):
        ...
        with torch.no_grad():
            model_output = model(**encoded_input) # model_output[0] : (batch_size, seq_length, hidden_size)
            
        embeddings = mean_pooling(model_output, encoded_input['attention_mask']) # (batch_size, hidden_size)
        embeddings = torch.nn.functional.normalize(embeddings)
        
        scores = torch.matmul(embeddings, polar_vector) # (batch_size, hidden_size) @ (hidden_size) -> (batch_size)
...
def main():
    ...
    for idx, doc in tqdm(enumerate(nlp.pipe(texts, batch_size=300)), total=len(texts), desc="Tokenisation"):
        if doc.text.strip():
