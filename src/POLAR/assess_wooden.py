...
def setup_nltk():
    """Downloads required NLTK data for sentence tokenization."""
    try:
...
def embed_sentences(sentences: list, tokenizer, model, device, batch_size=32) -> torch.Tensor:
    ...
    for i in range(0, len(sentences), batch_size):
        ...
        encoded_input = tokenizer(
            ...
            return_tensors='pt', 
            max_length=512
        ).to(device) # {(input_ids: (batch_size, sequence_length), attention_mask:(batch_size, sequence_length)}
        
        with torch.no_grad():
            model_output = model(**encoded_input)
            
        embeddings = mean_pooling(model_output, encoded_input['attention_mask']) # (batch_size, sequence_length, hidden_size) -> (batch_size, hidden_size)
...
def build_polar_axis(pairs_path: Path, tokenizer, model, device) -> torch.Tensor:
    ...
    ldb_texts = [item["langue_de_bois"] for item in dataset if item.get("langue_de_bois")]
    ...
    embed_direct = embed_sentences(direct_texts, tokenizer, model, device)
    embed_ldb = embed_sentences(ldb_texts, tokenizer, model, device) 
    
    polar_vector = torch.mean(embed_ldb - embed_direct, dim=0) # mean over the number of sentences
    polar_vector = torch.nn.functional.normalize(polar_vector, p=2, dim=0)
    
    return polar_vector.to(device)

def compute_polar_scores_bulk(sentences: list, tokenizer, model, device, polar_vector, batch_size=32) -> np.ndarray:
    """Calculate the dot product directky on GPU :-> array 1D of scores."""
    model.eval()
    all_scores = []
    
    for i in tqdm(range(0, len(sentences), batch_size), desc="Computing sentence scores"):
        batch = sentences[i:i + batch_size]
        encoded_input = tokenizer(
        ...
        scores = torch.matmul(embeddings, polar_vector)
...
def main():
    ...
    parser.add_argument("--output-graph", type=str, default="logs/POLAR/ldb_evolution.png", help="Path to save the output graph.")
    ...
    if 'date' not in df.columns or 'raw_text' not in df.columns or 'affiliate political party' not in df.columns:
    ...
    plt.title("Evolution of 'Langue de Bois' in French Political Speeches", fontsize=16, pad=20)
    plt.ylabel("POLAR Projection Score (Langue de Bois)", fontsize=12)
    plt.xlabel("Year", fontsize=12)
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left', title="Political Party")
    plt.tight_layout()
    
    plt.savefig(args.output_graph, dpi=300)
    print(green(f"Analysis complete. Graph saved to {args.output_graph}"))
